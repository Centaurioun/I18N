...
public class WpfChecker : UserInterfaceChecker {
  ...
  static WpfChecker() { Register(new StandardCheckerExtension()); }
  ...
  /// <summary>
  /// Initializes a new instance of the WpfChecker class to be linked to an
  ...
  public override void DoTakeScreenshot(Issue issue) {
    ...
    if (host is Window) {
      ...
      windowRect =
          new System.Drawing.Rectangle((int)window.Left, (int)window.Top,
                                       (int)window.Width, (int)window.Height);
    } else {
      point = host.PointToScreen(new Point(0, 0));
      windowRect = new System.Drawing.Rectangle((int)point.X, (int)point.Y,
    ...
    System.Drawing.Bitmap screenshot =
        new System.Drawing.Bitmap(windowRect.Width, windowRect.Height);

    using (System.Drawing.Graphics graphics =
               System.Drawing.Graphics.FromImage(screenshot)) {
      // Take the screenshot
      ...
      if (HighlightIssues) {
        ...
        System.Drawing.Rectangle highlightRect = new System.Drawing.Rectangle(
            issue.HightlightRect.Left + dx, issue.HightlightRect.Top + dy,
            issue.HightlightRect.Width, issue.HightlightRect.Height);
        highlightRect.Inflate(HighlightMargin, HighlightMargin);

        graphics.DrawRectangle(
            new System.Drawing.Pen(GetHighlightColor(issue.IssueType),
                                   HighlightWidth),
            highlightRect);
        graphics.Flush();
      }
  ...
  private Size CalculateTextSize(Control control, string text) {
    ...
    var ft = new FormattedText(
        ...
        new Typeface(control.FontFamily, control.FontStyle, control.FontWeight,
                     control.FontStretch),
        control.FontSize, Brushes.Black, dpiScale.PixelsPerDip);

    return new Size(ft.Width, ft.Height);
  }

  private Issue CreateIssue(Type type, FrameworkElement element,
                            Rect hightlight) {
    ...
    issue.HightlightRect = new System.Drawing.Rectangle(
        (int)hightlight.Left, (int)hightlight.Top, (int)hightlight.Width,
        (int)hightlight.Height);
    ...
    while (thisElement != null) {
      ...
      if ((thisElement != null) &&
          (thisElement.GetType().ToString() ==
           "System.Windows.Forms.Integration.AvalonAdapter"))
        break;
    }

    issue.ScreenshotFileName = OutputDir + "\\" + ScreenshotFilePrefix + name +
                               "_" + issue.Name + ".png";
  ...
  private Rect GetRect(FrameworkElement element) {
    ...
    rectangleBounds = element.RenderTransform.TransformBounds(new Rect(
        location.X, location.Y, element.ActualWidth, element.ActualHeight));
  ...
  private void ProcessTruncation(FrameworkElement element, Size size) {
    var issue = (TruncationIssue)CreateIssue(typeof(TruncationIssue), element,
                                             GetRect(element));
  ...
  private void CheckListBox(ListBox listBox) {
    ...
    foreach (var value in listBox.Items) {
      var size = CalculateTextSize(
          listBox, value.ToString()); // + 2*GetSystemMetrics(SM_CXBORDER);
  ...
  private void CheckComboBox(ComboBox comboBox) {
    ...
    foreach (var value in comboBox.Items) {
      var size = CalculateTextSize(
          comboBox, value.ToString()); // + 2*GetSystemMetrics(SM_CXBORDER);
  ...
  private void CheckOverlap(FrameworkElement element) {
    if ((element.Parent == null) || IsDisabled(IssueType.Overlap) ||
        extensions.Ignore(element, new List<IssueType> { IssueType.Overlap }))
      return;
    ...
    var issue =
        (OverlapIssue)CreateIssue(typeof(OverlapIssue), element, highlightRect);
  ...
  private void CheckOutOfBounds(FrameworkElement element) {
    if ((element.Parent == null) || IsDisabled(IssueType.OutOfBounds) ||
        extensions.Ignore(element,
                          new List<IssueType> { IssueType.OutOfBounds }))
      return;

    FrameworkElement parent =
        (FrameworkElement)VisualTreeHelper.GetParent(element);
    ...
    if (((location.X + element.ActualWidth) > parent.ActualWidth) ||
        ((location.Y + element.Height) > parent.ActualHeight)) {
      var issue =
          CreateIssue(typeof(OutOfBoundsIssue), element, GetRect(element));
...
class StandardCheckerExtension : WpfCheckerExtension {
  ...
  public override bool Ignore(object control, List<IssueType> issueTypes) {
    return ((control is TabItem) || (control is Expander)) &&
           issueTypes.Contains(IssueType.Overlap);
  }
}
}
