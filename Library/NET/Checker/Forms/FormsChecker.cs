...
public class FormsChecker : UserInterfaceChecker {
  ...
  static FormsChecker() { Register(new StandardCheckerExtension()); }
  ...
  /// <summary>
  /// Initializes a new instance of the FormsChecker class to be linked to an
  /// existing checker object.
  /// </summary>
  ...
  /// <inheritdoc/>
  protected override CheckerExtensions GetExtensions() { return extensions; }
  ...
  /// <inheritdoc/>
  public override void Process(object root) { Process((Control)root); }
  ...
  public override void DoTakeScreenshot(Issue issue) {
    ...
    issue.ScreenshotFileName = OutputDir + "\\" + form.Name + "_" +
                               control.Name + "_" + issue.Name + ".png";

    // Get screenshot bitmap
    var screenshot = new Bitmap(form.Width, form.Height);

    // Take the screenshot
    form.DrawToBitmap(screenshot,
    ...
    if (HighlightIssues) {
      ...
      highlightRect.Location =
          new Point(highlightRect.Left + 8,
                    highlightRect.Top + SystemInformation.CaptionHeight + 8);
      ...
      graphics.DrawRectangle(
          new Pen(GetHighlightColor(issue.IssueType), HighlightWidth),
          highlightRect);
      graphics.Flush();
    }
  ...
  /// <summary>
  /// Abstract method that can be used to perform a special checking for certain
  /// controls.
  /// </summary>
  /// <param name="control">The control to be checked.</param>
  ...
  private void Process(Control control) {
    if (extensions.Ignore(control, new List<IssueType>()) ||
        ShouldSkipControl(control))
      return;
  ...
  private int CalculateTextWidth(Control control, string text) {
    return TextRenderer
        .MeasureText(text, control.Font, control.Size, TextFormatFlags.NoPrefix)
        .Width;
  }
  ...
  private Size CalculateTextSize(Control control, string text, int leftMargin) {
    ...
    size = TextRenderer.MeasureText(text, control.Font, size,
  ...
  private void CheckCheckBox(CheckBox checkBox) {
    ...
    var size = CalculateTextSize(checkBox, checkBox.Text,
  ...
  private void CheckRadioButton(RadioButton radioButton) {
    ...
    var size = CalculateTextSize(radioButton, radioButton.Text,
                                 GetSystemMetrics(SM_CXMENUCHECK));
  ...
  private void CheckButton(Button button) {
    ...
    var size =
        CalculateTextSize(button, button.Text, 4 * GetSystemMetrics(SM_CXEDGE));
  ...
  private void CheckComboBox(ComboBox comboBox) {
    ...
    foreach (var value in comboBox.Items) {
      Size size = CalculateTextSize(comboBox, comboBox.GetItemText(value),
  ...
  private void CheckListBox(ListBox listBox) {
    ...
    foreach (var value in listBox.Items) {
      Size size = CalculateTextSize(listBox, listBox.GetItemText(value),
                                    2 * GetSystemMetrics(SM_CXBORDER));
  ...
  private void CheckCheckedListBox(CheckedListBox checkedListBox) {
    ...
    foreach (var value in checkedListBox.Items) {
      var size =
          CalculateTextSize(checkedListBox, checkedListBox.GetItemText(value),
                            +GetSystemMetrics(SM_CXMENUCHECK) +
                                2 * GetSystemMetrics(SM_CXBORDER));
  ...
  private void ProcessTruncation(Control control, Size size) {
    var issue = (TruncationIssue)CreateIssue(typeof(TruncationIssue), control,
                                             control.Bounds);
  ...
  private void CheckNoName(Control control) {
    if ((control.Parent == null) || IsDisabled(IssueType.NoName) ||
  ...
  private void CheckDuplicateName(Control control) {
    if ((control.Parent == null) || (control.Name == "") ||
        IsDisabled(IssueType.DuplicateName) ||
        extensions.Ignore(control,
                          new List<IssueType> { IssueType.DuplicateName }))
      return;
  ...
  private void CheckOverlap(Control control) {
    if ((control.Parent == null) || IsDisabled(IssueType.Overlap) ||
        extensions.Ignore(control, new List<IssueType> { IssueType.Overlap }))
      return;
    ...
    foreach (Control thisControl in control.Parent.Controls)
      if ((thisControl != control) &&
          (control.Bounds.IntersectsWith(thisControl.Bounds))) {
    ...
    var issue =
        (OverlapIssue)CreateIssue(typeof(OverlapIssue), control, highlightRect);
  ...
  private void CheckOutOfBounds(Control control) {
    if ((control.Parent == null) || IsDisabled(IssueType.OutOfBounds) ||
        extensions.Ignore(control,
                          new List<IssueType> { IssueType.OutOfBounds }))
      return;

    if (((control.Location.X + control.Width) >
         control.Parent.ClientSize.Width) ||
        ((control.Location.Y + control.Height) >
         control.Parent.ClientSize.Height)) {
      var issue =
          CreateIssue(typeof(OutOfBoundsIssue), control, control.Bounds);
...
public class FormsCheckerExtension : CheckerExtension {
  /// <summary>
  /// Find a typed ancestor. It is either a direct parent or any parent above
  /// the direct parent that matches the given type.
  /// </summary>
  /// <param name="control">The control.</param>
...
class StandardCheckerExtension : FormsCheckerExtension {
  public override bool Show(UserInterfaceChecker checker, object control,
