...
namespace Soluling {
/// <summary>
/// Enumeration that specifies the plural form. There are six different plural
/// forms from singular to various plurals. How many forms a language uses
/// depends on the language. Most languages only use singular and plural.
/// </summary>
public enum Plural {
  ///< summary>Nullar. Used when count is zero. Few languages support this but
  ///< you can include a =0 pattern into your pattern string if you want to
  ///< handle 0 in a different way.</summary>
  Zero,
  ///< summary>Singular. Used when count is one. Most languages support
  ///< this.</summary>
  One,
  ///< summary>Dual. Used when count is two. Few languages support
  ///< this.</summary>
  Two,
  ///< summary>Trial, paucal, sexal, minority plural or plural-100. Used when
  ///< count is few. Few languages support this. The range depends on the
  ///< language. Most often this is something between 2 and 4.</summary>
  Few,
  ///< summary>Greater paucal. Used when count is many. Few languages support
  ///< this. The range depends on the language. Most often this is more
  ///< than 4.</summary>
  Many,
  ///< summary>Plural or universal. Used when count does not belong to any other
  ///< group. All languages support this. Most often this is the plural
  ///< form.</summary>
  Other
}
;
...
/// <summary>
/// Enumeration that specifies the gender form. There are three different gender
/// forms. How many forms a language uses depends on the language. Many
/// languages do not use gender so they only use other.
/// </summary>
public enum Gender {
  ...
  Female,
  ///< summary>Other or neutral. Used when language does not use gender or
  ///< gender is neutral. Most languages support this.</summary>
  Neutral
}
;
...
/// <summary>
/// Represents the method that will get the plural form matching the given
/// number.
/// </summary>
/// <param name="n">Absolute value of the source number (integer and decimals)
/// (e.g. 9.870 => 9.87)</param> <param name="i">Integer digits of n (e.g. 9.870
/// => 9)</param> <param name="v">Number of visible fraction digits in n, with
/// trailing zeros (e.g. 9.870 => 3)</param> <param name="w">Number of visible
/// fraction digits in n, without trailing zeros (e.g. 9.870 => 2)</param>
/// <param name="f">Visible fractional digits in n, with trailing zeros
/// (e.g. 9.870 => 870)</param> <param name="t">Visible fractional digits in n,
/// without trailing zeros (e.g. 9.870 => 87)</param> <returns>Plural form
/// function</returns>
...
public class PluralPattern : NumberPattern {
  ...
  /// <summary>
  /// Initializes a new instance of the PluralPattern class based on the
  /// specified pattern and plural type.
  /// </summary>
  /// <param name="value">Strings pattern that is used when below plural
  /// form.</param> <param name="plural">Plural form that is used.</param>
...
public class OperatorPattern : NumberPattern {
  ...
  /// <summary>
  /// Initializes a new instance of the OperatorPattern class based on the
  /// specified pattern and operator value.
  /// </summary>
  /// <param name="value">Strings pattern that is used when below operator
  /// value.</param> <param name="kind">Operator kind.</param> <param
  /// name="operand">Operator operand.</param> <param name="operand2">Second
  /// operator operand.</param>
  public OperatorPattern(string value, OperatorKind kind, int operand,
                         int operand2 = 0) {
    Value = value;
    Kind = kind;
    Operand = operand;
...
public class GenderPattern : Pattern {
  ...
  /// <summary>
  /// Initializes a new instance of the GenderPattern class based on the
  /// specified pattern and gender.
  /// </summary>
  /// <param name="value">Strings pattern that is used when below plural
  /// form.</param> <param name="gender">Gender that is used.</param>
...
public class SelectPattern : Pattern {
  ...
  /// <summary>
  /// Initializes a new instance of the SelectPattern class based on the
  /// specified pattern and select value.
  /// </summary>
  /// <param name="value">Strings pattern that is used when below select
  /// value.</param> <param name="select">Select value that is used.</param>
...
public class FormatPart {
  ...
  public IEnumerator<Pattern> GetEnumerator() { return items.GetEnumerator(); }
  ...
  public OperatorPattern FindOperator(uint value) {
    ...
    for (var i = 0; i < Count; i++) {
      ...
      if (pattern is OperatorPattern) {
        ...
        if ((result.Kind == OperatorKind.Around) &&
            (value >= result.Operand - MultiPattern.OperatorDelta) &&
            (value <= result.Operand + MultiPattern.OperatorDelta))
          return result;
      }
    }
    ...
    for (var i = 0; i < Count; i++) {
      ...
      if (pattern is OperatorPattern) {
        ...
        if ((result.Kind == OperatorKind.Range) && (value >= result.Operand) &&
            (value <= result.Operand2))
          return result;
      }
    }
    ...
    for (var i = 0; i < Count; i++) {
      ...
      if (pattern is OperatorPattern) {
        ...
        if (((thisPattern.Kind == OperatorKind.LessOrEqual) &&
                 (value <= thisPattern.Operand) ||
             ((thisPattern.Kind == OperatorKind.Less) &&
              (value < thisPattern.Operand)) ||
             ((thisPattern.Kind == OperatorKind.GreaterOrEqual) &&
                  (value >= thisPattern.Operand) ||
              ((thisPattern.Kind == OperatorKind.Greater) &&
               (value > thisPattern.Operand))) &&
                 ((result == null) || (Math.Abs(value - thisPattern.Operand) <
                                       Math.Abs(value - result.Operand))))) {
          result = thisPattern;
        }
      }
    }
  ...
  public PluralPattern Find(Plural plural) {
    foreach (var pattern in items)
      if ((pattern is PluralPattern) &&
          (((PluralPattern)pattern).Plural == plural))
  ...
  public GenderPattern Find(Gender gender) {
    foreach (var pattern in items)
      if ((pattern is GenderPattern) &&
          (((GenderPattern)pattern).Gender == gender))
  ...
  public SelectPattern Find(string select) {
    foreach (var pattern in items)
      if ((pattern is SelectPattern) &&
          (((SelectPattern)pattern).Select == select))
  ...
  /// <returns>true if the PluralParameter contains a pattern for the plural
  /// form; otherwise, false.</returns>
  public bool Exists(Plural plural) { return Find(plural) != null; }
  ...
  /// <returns>true if the PluralParameter contains a pattern for the gender;
  /// otherwise, false.</returns>
  public bool Exists(Gender gender) { return Find(gender) != null; }
  ...
  public OperatorPattern Add(string value, OperatorKind kind, int operand,
                             int operand2 = 0) {
...
public class FormatString {
  ...
  /// <summary>
  /// Initializes a new instance of the PluralString class based on the
  /// specified pattern.
  /// </summary>
  ...
  /// <summary>Optional start pattern. Used when there are more than one plural
  /// enabled parameters in a string.</summary>
    ...
    while (index < pattern.Length) {
      ...
      if (parameter.ParameterType == "plural") {
        if (MultiPattern.IsPluralForm(name))
          ...
          if (MultiPattern.StringToOperator(name, out operatorKind, out operand,
                                            out operand2))
  ...
  private bool ParseLegacy(string patterns) {
    ...
    while (i < patterns.Length) {
      ...
      if (str == NEXT) {
        ...
        selectStr = "";
      } else if ((selectStr == "") && (Count == 1) && (str.IndexOf('{') >= 0) &&
                 (StartPattern != "")) {
  ...
  private void ParsePatterns(string patterns) {
    if (!ParseIcu(patterns)) {
      ...
      if (!ParseLegacy(patterns))
        throw new Exception(
            String.Format("\"{0}\" is not a valid patterns strings", patterns));
    }
  }
}
...
/// </summary>
/// <remarks>
/// Most languages have only single and plural forms. However this is not case
/// with every language. Asian languages do not have plural form at all. They
/// only have single form. Most Slavic languages have three forms: singular,
/// plural and one between them (dual or paucal). Sometimes zero is handled as
/// plural, sometimes as singular. Some languages have four different forms.
/// Arabic has six different forms.
///
/// This is why you can not use the standard Format if you want to create
/// grammatically correct dynamic messages that contain counts. Instead you can
/// use Plural.Format function. It works little bit like GetText's ngettext
/// funtion. You pass combined pattern and count parameter. The functions
/// calculates what pattern to use based on the passed count parameter and
/// returns that pattern.
///
/// Plural.Format works in the same way as String.Format but instead of having
/// only one pattern in a string you will have one or more depending on the
/// grammatical rules of the language. English for example uses singular and
/// plural. it is very common to write something line this <code>str =
/// String.Format("{0} file(s)", fileCount);</code> This is very common but not
/// grammatically correct. The right way would to use either "{0} file" or "{0}
/// files" depending on the actual file count. Using this class you can include
/// all different pattern into one string by using the syntax the string
/// contains sub parts for each grammatical rule. The English string would be
/// "{files, plural, one {{0} file} other {{0} files}}" that contains two parts:
/// one and other. One is for singular form and other is for plural form.
/// <code>str = Plural.Format("{fileCount, plural, one {{0} file} other {{0}
/// files}}", fileCount);</code> You can also use the legacy format: <code>str =
/// Plural.Format("one;{0} file;other;{0} files", fileCount);</code> If you
/// pattern contains {, } or \ characters you must escape them <code>"Item {\}
/// {0} car(s)"</code> becomes to <code>"{cars, plural, one {Item \{\\\} {0}
/// car} other {Item \{\\\} {0} cars}}".</code> <para> Remember to end the
/// resource string item name with Plural such as MyStringPlural. That way
/// Soluling can identify the pattern as multi pattern string and it can enable
/// advanced plural translation editor.
/// </para>
/// <para>
/// Plural engine need to know the language that the application uses. Compiled
/// application file does not contain this information. This is why you should
/// create a resource string that contains the locale id and set that value to
/// <see cref="Language.Id"/>. A good place to set it is the constructor of your
/// application.
/// </para>
/// </remarks>
/// <example>
/// Following example show how to create grammatically correct "{0} file(s)"
/// message. English uses singular and plural so we need one and other parts in
/// the pattern. <code> int count;
/// ...
/// textLabel.Text = Plural.Format("{files, plural, one {{0} file} other {{0}
/// files}}", count);
/// </code>
/// In a real application you would not hard code the pattern strings into the
/// code but use resource strings. <code> int count;
/// ...
/// textLabel.Text = Plural.Format(Properties.Resources.MessageStringPlural,
/// count);
/// </code>
/// If you want to handle 0 in a different way you can add zero part into any
/// pattern on any language even if the language does not use different grammar
/// in the case of zero. <code> int count;
/// ...
/// textLabel.Text = Plural.Format("{files, plural, zero {No files} one {{0}
/// file} other {{0} files}}", count);
/// </code>
/// </example>
public static class MultiPattern {
  private static Dictionary<string, PluralProc> procs =
      new Dictionary<string, PluralProc>();
  ...
  /// </summary>
  /// <returns>Plural index procedure.</returns>
  public static PluralProc GetProc() { return GetProc(Language.Culture); }
  ...
  /// </summary>
  /// <param name="manager">Resource manager that is used to find
  /// resources.</param> <param name="name">Name of the resource containing the
  /// pattern string.</param> <param name="count">The number to format.</param>
  /// <returns>A copy of format string matching the plural form of specified
  /// number.</returns>
  public static string GetPattern(ResourceManager manager, String name,
                                  uint count) {
  ...
  /// <returns>true if the string value could be converted into plural
  /// form.</returns>
  ...
  /// </summary>
  /// <param name="value">String name of the operator value such as
  /// "=1".</param> <param name="kind">Returns the operator kind.</param> <param
  /// name="operand">Returns the operand value.</param> <param
  /// name="operand2">Returns the operand value.</param> <returns>true if the
  /// string value could be converted into operator value.</returns>
  public static bool StringToOperator(string value, out OperatorKind kind,
                                      out int operand, out int operand2) {
  ...
  /// <returns></returns>
  public static bool TryStringToOperator(string value, out OperatorKind kind,
                                         out int operand, out int operand2) {
    kind = OperatorKind.Equal;
    operand2 = 0;
    ...
    if (p > 0) {
      ...
      return int.TryParse(value.Substring(0, p), out operand) &&
             int.TryParse(value.Substring(p + 2), out operand2);
  ...
  /// <returns>true if the string value could be converted into
  /// gender.</returns>
  ...
  public static Plural StringToPlural(string value) {
    ...
    if (!StringToPlural(value, out result))
      throw new Exception(
          String.Format("Invalid plural form value: {0}", value));
  ...
  /// </summary>
  /// <param name="value">String name of the operator value such as
  /// "=1".</param> <param name="kind">Operator kind.</param> <returns>Operand
  /// value.</returns>
  ...
  /// <param name="count">The number to format.</param>
  /// <returns>A string in which the format items have been replaced by the
  /// string representation of the corresponding objects in args.</returns>
  ...
  /// format.</param> <returns>A string in which the format items have been
  ...
  public static string Format(string patterns, int count,
    ...
    var result = String.Format(
        GetPattern(patterns, (uint)count, out startPattern), args);
  ...
  /// replaced by the string representation of the corresponding objects in
  /// args.</returns>
  public static string Format(string patterns, Gender gender,
                              params object[] args) {
    string startPattern;
    var result =
        String.Format(GetPattern(patterns, gender, out startPattern), args);
  ...
  /// replaced by the string representation of the corresponding objects in
  /// args.</returns>
  public static string Format(string patterns, string select,
                              params object[] args) {
    string startPattern;
    var result =
        String.Format(GetPattern(patterns, select, out startPattern), args);
  ...
  /// replaced by the string representation of the corresponding objects in
  /// args.</returns>
  public static string Format(ResourceManager manager, string name, int count,
                              params object[] args) {
  ...
  /// </summary>
  /// <param name="patterns">A plural and/or gender enabled composite format
  /// string.</param> <param name="args">Count and gender parameters for the
  /// patterns.</param> <returns>A string in which the format items have been
  /// replaced by the string representation of the corresponding objects in
  /// args.</returns>
  public static string FormatMulti(string patterns, params object[] args) {
    ...
    if (count != str.Count)
      throw new Exception(String.Format(
          "Pattern \"{0}\" does not have enough patterns for {1} parameter",
          patterns, count));
    ...
    if ((str.StartPattern != "") && (str.Syntax == FormatMessageSyntax.Icu)) {
      ...
      for (var i = 0; i < str.Count; i++) {
        ...
        if (IsNumber(arg)) {
          ...
          if (!genderFound)
            throw new Exception(
                String.Format("No gender parameter found for: {0}", patterns));
          else if (!valueFound)
            throw new Exception(
                String.Format("No value parameter found for: {0}", patterns));
  ...
  private static Plural GetDefaultPlural(decimal n, int i, int v, int w, int f,
                                         int t) {
    if (i == 1)
      return Plural.One;
    else
