...
/// <summary>
/// Class that contains one decoded string that is the string id and the string
/// value.
/// </summary>
...
public class HiddenId {
  ...
  private Dictionary<ZeroWidthCharacter, object> value =
      new Dictionary<ZeroWidthCharacter, object>();

  /// <summary>
  ///
  /// </summary>
  public HiddenId() { SetDefault(); }
  ...
  private void ValueChanged() {
    ...
    foreach (ZeroWidthCharacter c in (ZeroWidthCharacter[])Enum.GetValues(
                 typeof(ZeroWidthCharacter))) {
  ...
  public string Inject(string value, int id) { return value + Encode(id); }
  ...
  /// <summary>
  /// Parse a string that may contain injected id. If the string contains
  /// multiple strings return the first one.
  /// </summary>
  /// <param name="str">String that may contain injected id.</param>
  ...
  private DecodedString Add(List<DecodedString> list, StringBuilder value,
                            StringBuilder id) {
    var result = new DecodedString() { Value = value.ToString(),
