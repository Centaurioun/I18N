...
/// </summary>
/// <remarks>
/// Abbreviation rules use CLDR syntax where there is a set of rules for each
/// language and abbreviation type.
/// </remarks>
...
class Data {
    public AbbreviationRule[] Long {
        get;
        set;
    }
    public AbbreviationRule[] Short {
        get;
        set;
    }
    public AbbreviationRule[] Currency {
        get;
        set;
    }
}
...
public static class AbbreviatedNumber {
    ...
    /// <param name="precision">Number of digits that are used to express a
    /// value.</param> <returns>The formatted string.</returns>
    public static string Format(AbbreviatedNumberForm form, double value,
    ...
    public static string FormatShort(double value,
                                     int precision = DEFAULT_PRECISION) {
        return Format(value, AbbreviatedNumberForm.Short, precision,
                      GetData().Short);
    }
    ...
    public static string FormatCurrency(double value,
                                        int precision = DEFAULT_PRECISION) {
        return Format(value, AbbreviatedNumberForm.Currency, precision,
                      GetData().Currency);
    }
    ...
    /// <param name="longRules">Rules to abbreviate a number to a long
    /// form.</param> <param name="shortRules">Rules to abbreviate a number to a
    /// short form.</param> <param name="currencyRules">Rules to abbreviate a
    /// number to a currency form.</param>
    public static void Register(string id, AbbreviationRule[] longRules,
                                AbbreviationRule[] shortRules,
                                AbbreviationRule[] currencyRules) {
...
#pragma warning restore IDE0018 // Inline variable declaration
    ...
    private static string GetStr(double value, int precision) {
        ...
        return DeleteTrailingZeros(
                   value.ToString("F" + Math.Max(0, precision - valuePrecision)));
    }

    private static string GetNotAbbreviatedStr(double value,
    ...
    private static AbbreviationRule Find(ulong range, Plural plural,
                                         AbbreviationRule[] rules) {
    ...
    private static string Format(double value, AbbreviatedNumberForm form,
                                 int precision, AbbreviationRule[] rules) {
        foreach (var rule in rules) {
            if (rule.Range <= value) {
                ...
                if (displayValue == count) {
                    ...
                    if (plural != rule.Plural) {
                        // Wrong plural form. Find the correct one and get abbreviation
                        // again
