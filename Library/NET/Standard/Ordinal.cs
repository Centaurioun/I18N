...
/// For example PluralGroup.Japanese is not only applied to Japanese but all the
/// other languages where plural rules are the same as in Japanese (expecially
/// Asian ones).
/// </remarks>
...
public delegate string OrdinalLongProc(SmallOrdinal ordinal, Plural plural,
                                       Gender gender);
...
public delegate string OrdinalShortProc(uint ordinal, Plural plural,
                                        Gender gender);

class OrdinalData {
    public OrdinalLongProc LongProc {
        get;
        set;
    }
    public OrdinalShortProc ShortProc {
        get;
        set;
    }
}
...
/// </summary>
/// <remarks>
/// For example in English "This is my 1st car" and "This is my 2nd card". Note
/// the ending after the number. It is either st, nd, rd, or th depending on the
/// number. Some other countries use different endings and the rules vary. Some
/// countries just add pediod in the end like in Finnish "Tämä on minun 1.
/// autoni" or "Tämä on minun 2. autoni".
/// </remarks>
public static class Ordinal {
    private static Dictionary<string, OrdinalData> datas =
        new Dictionary<string, OrdinalData>();
    ...
    public static string Format(OrdinalStringForm form, uint ordinal,
    ...
    /// <summary>
    /// Converts an ordinal number to a string using the short format such as
    /// "1st" and "4th".
    /// </summary>
    /// <param name="ordinal">Ordinal number.</param>
    /// <param name="plural">Specifies the plural form.</param>
    /// <param name="gender">Specifies the gender.</param>
    /// <returns>The formatted string.</returns>
    public static string FormatShort(uint ordinal, Plural plural = Plural.One,
                                     Gender gender = Gender.Neutral) {
        return GetData().ShortProc(ordinal, plural, gender);
    }

    /// <summary>
    /// Converts an ordinal number to a string using the long format such as
    /// "first" and "fourth".
    /// </summary>
    /// <remarks>
    /// Only the first 10 ordinals can be formatted to the long format. The rest
    /// of the ordinals will always be formatted using the short format such as
    /// "21st" and "24th".
    /// </remarks>
    /// <param name="ordinal">Ordinal number.</param>
    /// <param name="plural">Specifies the plural form.</param>
    /// <param name="gender">Specifies the gender.</param>
    /// <returns>The formatted string.</returns>
    public static string FormatLong(uint ordinal, Plural plural = Plural.One,
                                    Gender gender = Gender.Neutral) {
    ...
    /// <param name="longProc">Function that converts a small ordianal to a long
    /// string.</param> <param name="shortProc">Function that converts an ordianal
    /// to a short string.</param>
    public static void Register(string id, OrdinalLongProc longProc,
                                OrdinalShortProc shortProc) {
        OrdinalData data;
    ...
    private static OrdinalData GetData() {
        ...
        if (datas.TryGetValue(Language.Id, out data))
            ...
            if (datas.TryGetValue(Language.Culture.TwoLetterISOLanguageName,
                                  out data))
                return data;
            else
                return datas[""];
    ...
    private static bool IsShortOrdinal(uint value) {
        return (value >= (uint)SmallOrdinal.First) &&
               (value <= (uint)SmallOrdinal.Tenth);
    }

    private static string GetDefaultShort(uint ordinal, Plural plural,
                                          Gender gender) {
        return ordinal.ToString();
    }

    private static string GetDefaultLong(SmallOrdinal ordinal, Plural plural,
                                         Gender gender) {
        return ordinal.ToString();
    }

    private static string GetPeriodShort(uint ordinal, Plural plural,
                                         Gender gender) {
        return ordinal.ToString() + ".";
    }

    // English
    private static string GetEnglishShort(uint ordinal, Plural plural,
                                          Gender gender) {
        string str;
    ...
    private static readonly string[] ENGLISH = { "",      "first",   "second",
                                                 "third", "fourth",  "fifth",
                                                 "sixth", "seventh", "eighth",
                                                 "ninth", "tenth"
                                               };

    private static string GetEnglishLong(SmallOrdinal ordinal, Plural plural,
                                         Gender gender) {
    ...
    private static string GetGermanLong(SmallOrdinal ordinal, Plural plural,
                                        Gender gender) {
    ...
    // Dutch
    private static string GetDutchShort(uint ordinal, Plural plural,
                                        Gender gender) {
        return ordinal.ToString() + "e";
    }

    private static readonly string[] DUTCH = { "",        "eerste",  "tweede",
                                               "derde",   "vierde",  "vijfde",
                                               "zesde",   "zevende", "achtste",
                                               "negende", "tiende"
                                             };

    private static string GetDutchLong(SmallOrdinal ordinal, Plural plural,
                                       Gender gender) {
        return DUTCH[(int)ordinal];
    }

    // French
    private static string GetFrenchShort(uint ordinal, Plural plural,
                                         Gender gender) {
        string str;
    ...
    private static string GetFrenchLong(SmallOrdinal ordinal, Plural plural,
                                        Gender gender) {
        if (ordinal == SmallOrdinal.First) {
            if (gender == Gender.Male)
                return "premier";
    ...
    private static readonly string[] FINNISH_PLURALS = {
        "",           "ensimmäiset", "toiset",     "kolmannet",    "neljännet",
        "viidennet",  "kuudennet",   "seitsennet", "kahdeksannet", "yhdeksännet",
        "kymmenennet"
    };

    private static string GetFinnishLong(SmallOrdinal ordinal, Plural plural,
                                         Gender gender) {
    ...
    private static readonly string[] ESTONIAN_PLURALS = {
        ...
        "kuues", "seitsmes", "kaheksas", "üheksas", "kümnes"
    };

    private static string GetEstonianLong(SmallOrdinal ordinal, Plural plural,
                                          Gender gender) {
        return ESTONIAN_PLURALS[(int)ordinal];
    }

    // Danish
    private static readonly string[] DANISH = { "",       "første",  "anden",
                                                "tredje", "fjerde",  "femte",
                                                "sjette", "syvende", "ottende",
                                                "niende", "tiende"
                                              };

    private static string GetDanishLong(SmallOrdinal ordinal, Plural plural,
                                        Gender gender) {
        return DANISH[(int)ordinal];
    }

    // Swedish
    private static readonly string[] SWEDISH = { "",       "första", "andra",
                                                 "tredje", "fjärde", "femte",
                                                 "sjätte", "sjunde", "åttonde",
                                                 "nionde", "tionde"
                                               };

    private static string GetSwedishLong(SmallOrdinal ordinal, Plural plural,
                                         Gender gender) {
    ...
    private static readonly string[] NORWEGIAN_BOKMAL = {
        "",       "første",  "annen",   "tredje", "fjerde", "femte",
        "sjette", "sjuende", "åttende", "niende", "tiende"
    };

    private static string GetNorwegianBokmalLong(SmallOrdinal ordinal,
    ...
    private static readonly string[] NORWEGIAN_NYNORSK = {
        ...
        "sjette", "sjuande", "åttande", "niande", "tiande"
    };

    private static string GetNorwegianNynorskLong(SmallOrdinal ordinal,
            Plural plural, Gender gender) {
        return NORWEGIAN_NYNORSK[(int)ordinal];
    }

    // Icelandic
    private static readonly string[] ICELANDIC = { "",       "fyrsti",  "annar",
                                                   "þriðji", "fjórði",  "fimmti",
                                                   "sjötti", "sjöundi", "áttundi",
                                                   "níundi", "tíundi"
                                                 };

    private static string GetIcelandicLong(SmallOrdinal ordinal, Plural plural,
                                           Gender gender) {
        return ICELANDIC[(int)ordinal];
    }

    // Japanese
    private static readonly string[] JAPANESE = { "",       "一つ目", "二つ目",
                                                  "三つ目", "四つ目", "五つ目",
                                                  "六つ目", "七つ目", "八つ目",
                                                  "九つ目", "十"
                                                };

    private static string GetJapaneseLong(SmallOrdinal ordinal, Plural plural,
                                          Gender gender) {
        return JAPANESE[(int)ordinal];
    }

    private static string GetJapaneseShort(uint ordinal, Plural plural,
                                           Gender gender) {
        if (IsShortOrdinal(ordinal))
    ...
    // Korean
    private static readonly string[] KOREAN = { "",       "첫째",   "둘째",
                                                "셋째",   "넷째",   "다섯째",
                                                "여섯째", "일곱째", "여덟째",
                                                "아홉째", "열째"
                                              };

    private static string GetKoreanLong(SmallOrdinal ordinal, Plural plural,
                                        Gender gender) {
        return KOREAN[(int)ordinal];
    }

    private static string GetKoreanShort(uint ordinal, Plural plural,
                                         Gender gender) {
        if (IsShortOrdinal(ordinal))
    ...
    private static string GetSimplifiedChineseLong(SmallOrdinal ordinal,
            Plural plural, Gender gender) {
        return SIMPLIFIED_CHINESE[(int)ordinal];
    }

    private static string GetSimplifiedChineseShort(uint ordinal, Plural plural,
            Gender gender) {
        if (IsShortOrdinal(ordinal))
