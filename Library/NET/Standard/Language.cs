...
/// <summary>
/// Implements current locale and resource cultures. Provides methods to get
/// list of available languages for the application. Also provides methods to
/// get resource data that has been added as Resource or Embedded Resource.
/// </summary>
/// <remarks>
/// <para>
/// If you want to create a custom user interface to show available languages
/// for the application use this class to the list of the available languages
/// for the application.
/// </para>
/// <para>
/// ResourceManager performs language cascading where the resource is first
/// looked from the resource of the specific culture. If not found them the
/// search is continue from the parent culture all the way to neutral culture
/// (e.g. the resource in the original assembly file). Unfortunately
/// ResourceManager only works with resources that are in the .resx file. If you
/// have added a file as Resource or Embedded Resource into your project
/// ResourceManager cannot access them. In that case use this class.
/// </para>
/// </remarks>
/// <example>
/// <para>
/// The following gets the list of available languages for the application and
/// uses it to populate the language selection user interface.
/// </para>
/// <code>
...
/// </code>
/// <para>
/// The following example loads a standard resource and an embedded resource
/// into picture boxes.
/// </para>
/// <code>
/// Soluling.Language.SetInitial();
///
/// resourcePictureBox.Image = new
/// Bitmap(Resources.GetStandardResource("airplane.png"));
/// embeddedResourcePictureBox.Image = new
/// Bitmap(Resources.GetEmbeddedResource("car_sedan_blue.png"));
/// </code>
/// </example>
public class Language {
  ...
  /// </summary>
  /// <remarks>
  /// Original language is read from the value NeutralResourcesLanguage
  /// attribute. If there are no NeutralResourcesLanguage attribute then English
  /// (en) is returned.
  /// </remarks>
  ...
  /// </summary>
  /// <param name="culture">Locale id. If null use the default locale set in
  /// Control Panel.</param> <seealso
  /// cref="M:Soluling.Language.SetInitial(System.String)"/>
  ...
  /// <seealso
  /// cref="M:Soluling.Language.SetInitial(System.Globalization.CultureInfo)"/>
  ...
  private static void CheckActiveAssembly() {
    ...
    // Get the active culture. Get satellite assembly files matching the current
    // culture. If not found try to get the assembly of the parent culture and
    // so on.
    activeAssembly = null;
    CultureInfo cultureInfo = Thread.CurrentThread.CurrentUICulture;

    while ((activeAssembly == null) && (cultureInfo != null) &&
           (cultureInfo.Name != "")) {
      try {
  ...
  /// <returns>Culture info of the resource item. null if there are no localized
  /// value of the resource but only the original language independent
  /// resource.</returns>
  public static CultureInfo GetResourceCulture(string baseName,
                                               Assembly assembly,
                                               string resourceName,
                                               CultureInfo culture = null) {
    return GetResourceCulture(new ResourceManager(baseName, assembly),
  ...
  /// resources.</param> <param name="resourceName">Specifies the resource item
  /// name.</param> <param name="culture">Culture that is used as the start
  /// culture.</param> <returns>Culture info of the resource item. null if there
  /// are no localized value of the resource but only the original language
  /// independent resource.</returns>
  public static CultureInfo GetResourceCulture(ResourceManager manager,
                                               string resourceName,
                                               CultureInfo culture = null) {
    ...
    /*
          ...
          while (culture != null)
          {
            ResourceSet resourceSet = manager.GetResourceSet(culture, true,
      false);
  ...
  private static string GetIdResourceName(string resourceName, string id) {
    ...
    if (p >= 0)
      resourceName = resourceName.Substring(0, p + 1) + id +
                     resourceName.Substring(p, resourceName.Length - p);
  ...
  private static string GetLanguageResourceName(string resourceName) {
    return GetIdResourceName(resourceName,
  ...
  public static Stream GetEmbeddedResource(string resourceName) {
    ...
    if (activeAssembly != mainAssembly) {
      CultureInfo culture =
          new AssemblyName(activeAssembly.FullName).CultureInfo;
      ...
      Stream value = activeAssembly.GetManifestResourceStream(
          GetIdResourceName(resourceName, culture.ToString()));
      ...
      if (culture.ToString() != culture.TwoLetterISOLanguageName) {
        value = activeAssembly.GetManifestResourceStream(
            GetIdResourceName(resourceName, culture.TwoLetterISOLanguageName));
      ...
      if (mainAssembly != null)
        return mainAssembly.GetManifestResourceStream(
            GetResourceName(resourceName));
      else
        return null;
    } else {
  ...
  private static Stream GetStandardResource(Stream resourceStream,
                                            string resourceName) {
  ...
  public static Stream GetStandardResource(string resourceName) {
    ...
    Stream result =
        GetStandardResource(GetEmbeddedResource("g.resources"), resourceName);

    if (result != null)
      return result;

    if (mainAssembly != activeAssembly)
      return GetStandardResource(mainAssembly.GetManifestResourceStream(
                                     GetResourceName("g.resources")),
                                 resourceName);
    else
      return null;
  }
  ...
  public static void CheckSatelliteAssemblyCount() {
    if (GetAvailableLanguages().Length == 0)
      throw new Exception(
          "In order to change language you must first create satellite assembly files for the application.");
  }
  ...
  /// </summary>
  /// <remarks>
  /// Use this method to get list of available language if you want to build
  /// your own user interface to show available languages and to select a new
  /// language. You can also use the built-in language selection dialogs. If you
  /// use Windows Forms use the <see cref="T:Soluling.Forms.SelectLanguage"/>
  /// class. If you use WPF use the <see cref="T:Soluling.WPF.SelectLanguage"/>
  /// class.
  /// </remarks>
  ...
  /// </summary>
  /// <param name="fileName">Filename of the assembly whose satellite assemblies
  /// are listed</param> <returns>Array of culture infos of the satellite
  /// assemblies.</returns>
  public static CultureInfo[] GetAvailableLanguages(string fileName) {
    ...
    string satelliteFileName =
        Path.GetFileNameWithoutExtension(fileName) + SATELLITE_EXT;
    ...
    for (int i = 0; i < dirs.Length; i++) {
      ...
      for (int j = 0; j < allCultures.Length; j++) {
        if ((allCultures[j].Name == str) ||
            (allCultures[j].TwoLetterISOLanguageName == str)) {
