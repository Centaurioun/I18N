...
public partial class Window1 : Window {
    ...
    public bool UseZero {
        get {
            return useZeroCheck.IsChecked.Value;
        }
    }

    private string Process(int completed, int total) {
        if (UseZero)
            return Soluling.MultiPattern.FormatMulti(
                       Properties.Resources.ZeroMessagePlural, completed, total);
        else
            return Soluling.MultiPattern.FormatMulti(
                       Properties.Resources.MessagePlural, completed, total);
    }
