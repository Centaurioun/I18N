...
public partial class Form1 : Form {
  ...
  private void UpdateItems() {
    ...
    using (
        OleDbConnection connection = new OleDbConnection(
            "Provider=Microsoft.ACE.OLEDB.15.0;Data Source=" + databaseFile)) {
      // If you get here the following exception:
      // "The 'Microsoft.ACE.OLEDB.15.0' provider is not registered on the local
      // machine." Make sure that Microsoft.ACE.OLEDB.15.0 is installed. If not
      // install Microsoft Access Database Engine 2013 Redistributable. If you
      // have some other version installed such as Microsoft.ACE.OLEDB.12.0 or
      // Microsoft.ACE.OLEDB.16.0 modify the above code. If you have 64-bit
      // machine but you have installed 32-bit Access open project properties,
      // select Build sheet and set platform target to x84.
      connection.Open();

      // Create a SQL statement that selects only those rows matching the
      // current culture
      string sql = String.Format(
          "SELECT Name, FieldPlayers, Goalie, Origin, Description FROM Sport WHERE LangId='{0}'",
          Language.Id);
  ...
  private void Form1_Load(object sender, EventArgs e) { UpdateItems(); }
