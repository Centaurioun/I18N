...
public partial class Form1 : Form {
  ...
  private void ProcessPluralUnaware(uint count, Label label) {
    ...
    if (MultiPattern.IsSingleFormLanguage() || (count == 1) ||
        ((count == 0) && MultiPattern.IsZeroLikeOne()))
  ...
  private void ProcessHomeBrewed(uint count, Label label) {
    // This works on some Western languages (those that use similar plural rules
    // as English) but would fail for example in French. Do not use code like
    // this!
    if (count == 1)
      label.Text = String.Format(Properties.Resources.File, count);
    else
    ...
    if (!MultiPattern.IsSingleFormLanguage() && (count == 0) &&
        MultiPattern.IsZeroLikeOne())
      label.ForeColor = Color.Red;
    else
      label.ForeColor = Color.Green;
  }

  // The following two samples handle plural forms correctly. Use this kind of
  // code in your applications.
  private void ProcessPluralAware(int count, Label label) {
    label.Text =
        MultiPattern.Format(Properties.Resources.FilesPlural, count, count);
    label.ForeColor = Color.Green;
  }

  private void ProcessMultiPlural(uint completed, int total, Label label) {
    label.Text = MultiPattern.FormatMulti(Properties.Resources.MessagePlural,
                                          completed, total);
    label.ForeColor = Color.Green;
  }
