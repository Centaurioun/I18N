...
public sealed partial class MainWindow : Window {
  ...
  public MainWindow() {
    ...
    // Use String.Format instead of ".." + value in order to make it possible to
    // localize dynamic messages correctly
    text3.Text = String.Format(rl.GetString("Name"), name);

    // Use DateTimeFormatter to show date as string
    var formatter =
        new DateTimeFormatter("shortdate", GlobalizationPreferences.Languages);
  ...
  private void myButton_Click(object sender, RoutedEventArgs e) {
    count++;
    // Use MultiPattern.Format instead of String.Format when you want to have
    // grammatically correct sentences
    myButton.Content =
        Soluling.MultiPattern.Format(rl.GetString("ClickedTimesPlural"), count);
  }
}
}
