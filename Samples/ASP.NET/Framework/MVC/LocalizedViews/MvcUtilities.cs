...
public class Utilities {
    ...
    public static string GetImageFile(HttpServerUtilityBase server,
                                      string fileName, string culture = "") {
        string localizedImage =
            "\\" +
            Path.Combine(Path.Combine(Path.GetDirectoryName(fileName), culture),
        ...
        if (File.Exists(server.MapPath(localizedImage)))
            ...
            if (index > 0) {
                ...
                localizedImage =
                    "\\" + Path.Combine(
                        Path.Combine(Path.GetDirectoryName(fileName), language),
                        Path.GetFileName(fileName));
...
public class LocalizedRazorViewEngine : RazorViewEngine {
    ...
    ViewEngineResult FindPartialView(ControllerContext controllerContext,
        ...
        var result = base.FindPartialView(controllerContext,
                                          localizedPartialViewName, useCache);
    ...
    public override ViewEngineResult FindView(ControllerContext controllerContext,
        ...
        var result = base.FindView(controllerContext, localizedViewName,
                                   localizedMasterName, useCache);
