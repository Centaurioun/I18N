...
public class SportsController : Controller {
  ...
  private void ProcessSport(Sport sport) {
    // If the language does not exist and machine translator exit, use it to
    // translate the sport to that language.
    var targetSportLanguage = sport.Languages.Find(m => m.Language == Language);

    if ((targetSportLanguage == null) && (machineTranslator != null) &&
        (sport.Languages.Count > 0)) {
      ...
      // Create a new language specific sport using the machine translated
      // properties
      targetSportLanguage =
          new SportLanguage { Language = Language, MachineTranslated = true,
                              Name = machineTranslator.Translate(
                                  originalSportLanguage.Name,
                                  originalSportLanguage.Language, Language),
                              Origin = machineTranslator.Translate(
                                  originalSportLanguage.Origin,
                                  originalSportLanguage.Language, Language),
                              Description = machineTranslator.Translate(
                                  originalSportLanguage.Description,
                                  originalSportLanguage.Language, Language) };

      // If the machine translation succeeded add the sport language to the list
      // and sav it to the DB
      if (!string.IsNullOrEmpty(targetSportLanguage.Name) &&
          !string.IsNullOrEmpty(targetSportLanguage.Origin) &&
          !string.IsNullOrEmpty(targetSportLanguage.Description)) {
  ...
  public IActionResult GetSport(int id) {
    ...
    var sport = context.Sport.Include(s => s.Languages)
  ...
  public async Task<IActionResult> GetSportLanguage(int id, string language) {
    ...
    var sport = await context.Sport.Include(s => s.Languages)
    ...
    var sportLanguage =
        sport.Languages.FirstOrDefault(l => l.Language == language);
  ...
  PostSportLanguage(int id, [FromBody] SportLanguage sportLanguage) {
    if (!ModelState.IsValid)
      return BadRequest(ModelState);
    ...
    return CreatedAtAction("PostSportLanguage", new { id = sportLanguage.Id },
                           sportLanguage);
  }
  ...
  public async Task<IActionResult> DeleteSportLanguage(int id,
                                                       string language) {
    var sportLanguage = await context.SportLanguage.SingleOrDefaultAsync(
        s => (s.SportId == id) && (s.Language == language));

    if ((sportLanguage == null) && (language.IndexOf('-') >= 0)) {
      var neutralLanguage = language.Split('-')[0];

      sportLanguage = await context.SportLanguage.SingleOrDefaultAsync(
          s => (s.SportId == id) && (s.Language == neutralLanguage));
    }
