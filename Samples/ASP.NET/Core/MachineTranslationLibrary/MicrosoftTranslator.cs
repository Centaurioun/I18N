...
public class MicrosoftTranslator : LanguageMachineTranslator {
  private static List<EngineLanguageData> languageDatas =
      new List<EngineLanguageData>();
  ...
  public override string ToString() { return "Microsoft"; }
    ...
    using (var client = GetHttpClient()) {
      ...
      try {
        ...
        foreach (var item in translations) {
          ...
          Languages.Add(
              new Language { Id = MicrosoftIdToId(item.Key),
                             Name = language["name"].Value<string>() });
        }
      } catch (Exception) {
        Languages.Clear();
      }
  ...
  public override string[] Translate(string[] strings, string formId,
                                     string toId) {
  ...
  private UriBuilder GetUriBuilder(string command) {
    return new UriBuilder(
        $"https://api.cognitive.microsofttranslator.com/{command}?api-version=3.0");
  }
